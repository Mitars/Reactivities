    ...
    public class List
    {
        ...
        public class Handler : IRequestHandler<Query, List<Profile>>
        {
            ...
            public async Task<List<Profile>> Handle(Query request, CancellationToken cancellationToken)
            {
                ...
                {
                    case "following":
                        return (await Task.WhenAll(user.Followings.Select(async u => await this.profileReader.ReadProfile(u.UserName)))).ToList();
                    case "followers":
                        return (await Task.WhenAll(user.Followers.Select(async u => await this.profileReader.ReadProfile(u.UserName)))).ToList();
                    default:
                        throw new RestException(HttpStatusCode.BadRequest, new { Predicate = "Must be either following or followers" });
                }
            }
        }
    }
